...
let launch (asm: string) (warnOnAssertFailure: bool): int =
    ...
    try
        ...
        Log.debug (lazy $"Created temporary directory: %s{tmpDir}")
        let asmFile = System.IO.Path.Combine [| tmpDir; "code.asm" |]
        try
            System.IO.File.WriteAllText(asmFile, asm)
            Log.debug (lazy $"Saved assembly code in: %s{asmFile}")
            ...
            try
                Log.info (lazy
                            let formatArgs x y = $"%s{x} %s{y}"
                            let acc = ""
                            $"Launching RARS: %s{p.StartInfo.FileName} %O{Seq.fold formatArgs acc p.StartInfo.ArgumentList}")
                p.Start() |> ignore
                p.WaitForExit()
                let stdErr = p.StandardError.ReadToEnd()
                let explain = explainExitCode p.ExitCode
                if (p.ExitCode = asmErrCode) || (p.ExitCode = simErrCode) then
                    Log.error (lazy ($"BUG: RARS exited with code %d{p.ExitCode} (%s{explain}) "
                               + $"while loading or running %s{asmFile}"))
                    Log.error (lazy $"- - - - - - - - - - RARS output starts here - - - - - - - - - -%s{Util.nl}%s{stdErr}")
                    Log.error (lazy $"- - - - - - - - - - RARS output ends here - - - - - - - - - - -")
                    ...
                    if p.ExitCode = RISCVCodegen.assertExitCode && warnOnAssertFailure then
                        Log.warning (lazy $"RARS exited with code: %d{p.ExitCode} (%s{explain})")
                    else
                        Log.info (lazy $"RARS exited with code: %d{p.ExitCode} (%s{explain})")
                    Log.debug (lazy $"RARS output:%s{Util.nl}%s{stdErr}")
                    try
                        Log.debug (lazy $"Removing temporary directory: %s{tmpDir}")
                        System.IO.Directory.Delete(tmpDir, true)
                        p.ExitCode
                    with e ->
                        Log.error (lazy $"Error deleting temporary directory %s{tmpDir}: %s{e.Message}")
                        1 // Non-zero exit code
                else
                    Log.error (lazy $"BUG: unexpected RARS exit code %d{p.ExitCode}")
                    p.ExitCode // Non-zero exit code
            with e ->
                Log.error (lazy $"Error launching RARS: %s{e.Message}")
                1 // Non-zero exit code
        with e ->
            Log.error (lazy $"Error writing file %s{asmFile}: %s{e.Message}")
            1 // Non-zero exit code
    with e ->
        Log.error (lazy $"Error creating temporary directory: %s{e.Message}")
        1 // Non-zero exit code
