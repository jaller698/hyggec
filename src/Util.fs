...
let parseFile (fileName: string): Result<AST.UntypedAST, string> =
    match (Lexer.tokenize fileName) with
    | Error msg ->
        Error msg
    | Ok tokens ->
        ...
        | Error (pos, msg) ->
            Error $"%s{pos.Filename}:%d{pos.Line}:%d{pos.Column}: %s{msg}"
        | Ok ast ->
            Log.info (lazy "Parsing succeeded.")
            Ok ast


/// Format a message (e.g. an error message) with a position in the given file.
let formatMsg (pos: AST.Position, msg: string): string =
    $"%s{pos.Main.Filename}:%s{pos.Format}: %s{msg}"
...
let rec mkTempDir (prefix: string): string =
    ...
    // IMPORTANT: this code must use a thread-safe random generator
